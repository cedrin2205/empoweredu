...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM ict12hands3 WHERE image_url = '$imageToDelete'";
    if (mysqli_query($conn, $deleteQuery)) {
        if (unlink("uploads/$imageToDelete")) {
            header("Location: ICT12hands3.php");
            exit();
        } else {
...
<html lang="en">
  ...
  <body>
        ...
        <section>
            ...
            <div class="contain">
                <form action="ICT12uploadhands3.php" method="post" enctype="multipart/form-data">
            ...
            <div class="gallery">
                ...
                include "ictdb12.php";
                $limit = 5; 
                $sql = "SELECT * FROM ict12hands3 ORDER BY id DESC LIMIT $limit";
                $res = mysqli_query($conn, $sql);

                if (mysqli_num_rows($res) > 0) {
                    while ($ict12hands3 = mysqli_fetch_assoc($res)) { ?>
                        <div class="alb">
                            <?php 
                            $fileExtension = pathinfo($ict12hands3['image_url'], PATHINFO_EXTENSION);
                            if ($fileExtension === 'pdf') {
                                echo '<img src="img/pdficon.png" alt="PDF File">';
                            } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                                echo '<img src="img/wordicon.png" alt="Word File">';
                            } else {
                                echo '<img src="uploads/'.$ict12hands3['image_url'].'" alt="Uploaded Image">';
                            }
                            ?>
                            <p><?php echo $ict12hands3['filename']; ?></p>
                            <a class="delete-btn" href="ict12hands3.php?delete_image=<?=$ict12hands3['image_url']?>">
                                <img src="img/remove.png" alt="Delete" width="1" height="1">
                            </a>
                            <a class="download-btn" href="uploads/<?=$ict12hands3['image_url']?>" download="<?=$ict12hands3['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "ict12hands3.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
